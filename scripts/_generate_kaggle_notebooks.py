...
KAGGLE_DATASET_PATH = "/kaggle/input/datasets/thedevastator/rsicd-image-caption-dataset"
...
def shared_setup_cells(session_label):
    ...
    return [
        cell_md(f"# RSICD Adapter-CLIP — Session {session_label}\n"
                ...
                f"1. **Data (required)**: Click **+ Add data** in the right panel, search `thedevastator/rsicd-image-caption-dataset`, click **Add**. The dataset will appear at exactly `/kaggle/input/datasets/thedevastator/rsicd-image-caption-dataset/`.\n"
                f"2. **Previous-session Datasets** (sessions 2-5 only): click **+ Add data** → add any `rsicd-adapter-s*` or `rsicd-fullfinetune` Datasets you saved earlier\n"
                f"3. Settings: **Accelerator = GPU P100 or T4**, **Internet = ON**\n"
                f"4. Click **Save Version → Save Output** at the end of this session\n"
                f"\n"
                f"Cell 2 will verify the dataset is at the exact expected path."),
        cell_code(
            "# === Install + clone + env ===\n"
...
def cell_convert_data():
    ...
    The data path is set by cell 2 (sanity check) via the RSICD_ARCHIVE env var.
    This cell just runs the conversion and verifies the result.
    """
    return cell_code(
        "# === Convert Kaggle CSV -> our standard layout (one-time per session) ===\n"
        "# RSICD_ARCHIVE is already set by cell 2 (sanity check).\n"
        "import os\n"
        "from pathlib import Path\n"
        "if not DATA_OK:\n"
        "    print(\"Skipping conversion: cell 2 (sanity check) did not find a dataset.\")\n"
        "    print(\"   Go back to cell 2 and fix the data attachment, then re-run.\")\n"
        "else:\n"
        "    print(f\"RSICD_ARCHIVE = {os.environ.get('RSICD_ARCHIVE', '<not set>')}\")\n"
        "    !python scripts/00b_prepare_rsicd.py\n"
        "    imgs = sorted(Path('data/raw/RSICD_images').glob('*.jpg'))\n"
        "    n_imgs = len(imgs)\n"
        "    print(f\"Extracted {n_imgs} jpg files.\")\n"
        "    if n_imgs < 10000:\n"
        "        print(f\"WARN: expected 10,921 images, got {n_imgs}\")\n"
        "        print(\"   Check the dataset path and re-run.\")"
    )
