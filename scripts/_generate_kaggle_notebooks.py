...
def shared_setup_cells(session_label):
    ...
    return [
        cell_md(f"# RSICD Adapter-CLIP — Session {session_label}\n"
                ...
                f"1. **Data** — pick ONE of these:\n"
                f"   - **Option A** (recommended): Click **+ Add data** in the right panel, search `rsicd-image-caption-dataset`, click **Add**\n"
                f"   - **Option B** (if you don't want to attach a Dataset): Download `train.csv`, `valid.csv`, `test.csv` from the thedevastator Kaggle dataset page, then in the Files panel (right), click **Upload** and drop the 3 files. They land in `/kaggle/working/`.\n"
