...
def shared_setup_cells(session_label):
    ...
    return [
        ...
        cell_code(
            ...
            "    print(f\"  CSVs:   {{p.name for p in data_path.glob('*.csv')}}\")\n"
            "else:\n"
            "    print(f\"WARN: dataset path {{data_path}} not found. Click '+ Add data' and add thedevastator/rsicd-image-caption-dataset\")"
        ),
    ]
...
def cell_convert_data():
    ...
    return cell_code(
        ...
        f"import os\n"
        f"os.environ[\"RSICD_ARCHIVE\"] = \"{KAGGLE_DATASET_PATH}\"\n"
